...
foreach (dbFetchRows("SELECT * FROM `ospf_instances` WHERE `ospfAdminStat` = 'enabled'") as $instance)
{
  ...
  $port_count_enabled = dbFetchCell("SELECT COUNT(*) FROM `ospf_ports` WHERE `ospfIfAdminStat` = ? AND `device_id` = ? AND `context_name`= ?",array('enabled',$instance['device_id'],$instance['context_name']));;
