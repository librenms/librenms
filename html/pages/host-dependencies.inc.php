    ...
    <table id="hostdeps" class="table table-hover table-condensed table-striped">
        <thead>
            <tr>
                ...
                <th data-column-id="parent" data-type="string">Parent Host</th>
...
<?php } ?>
    ...
    formatters: {
        "actions": function(column, row) {
            ...
            response += "<button type='button' class='"+buttonClass+"' aria-label='Delete' data-toggle='modal' data-target='#confirm-delete' data-device_id='"+row.id+"' data-device_parent ='"+row.parentid+"' data-host_name='"+row.hostname+"' name='delete-host-dependency'><i class='fa fa-trash' aria-hidden='true'></i></button>";
            return response;
        }
    },
