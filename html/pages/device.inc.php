...
#  print_r($entity_state);
  ...
  <li class="' . $select['overview'] . '">
    ...
    if(!empty($vars['vrf-lite'])){
        $device_routing_count['bgp'] = dbFetchCell("SELECT COUNT(*) FROM bgpPeers BP join vrf_lite_cisco VR on VR.device_id=BP.device_id and BP.context_name=VR.context_name where  BP.device_id = ? and VR.vrf_name= ? ", array($device['device_id'],$vars['vrf-lite']));
        $device_routing_count['ospf'] = dbFetchCell("SELECT COUNT(*) FROM ospf_instances OI join vrf_lite_cisco VR on VR.device_id=OI.device_id and OI.context_name=VR.context_name where OI.ospfAdminStat = 'enabled' and BP.device_id = ? and VR.vrf_name= ?", array($device['device_id'],$vars['vrf-lite']));
