...
foreach (dbFetchRows("SELECT * FROM `bgpPeers` WHERE `device_id` = ? $extra_sql ORDER BY `bgpPeerRemoteAs`, `bgpPeerIdentifier`", array($device['device_id'])) as $peer) {
    ...
    echo '   <td width=20><span class=list-large>'.$i.'</span></td>
        ...
        Updates <i class='fa fa-arrow-down' style='color:".$config[theme_icon_colour]."' aria-hidden='true'></i> ".$peer['bgpPeerInUpdates']."
        <i class='fa fa-arrow-up' style='color:".$config[theme_icon_colour]."' aria-hidden='true'></i> ".$peer['bgpPeerOutUpdates'].'</td>
        </tr>
        <tr height=5></tr>';
