...
if (isset($_SESSION['authenticated']) && $_SESSION['authenticated']) {
    ...
    if (isset($_COOKIE['sess_id'], $_COOKIE['token']) && reauthenticate(clean($_COOKIE['sess_id']), clean($_COOKIE['token']))) {
        log_in_user();
...
if ((isset($username) && !$_SESSION['authenticated']) || isset($auth_error)) {
    $auth_message = isset($auth_error) ? $auth_error : 'Authentication Failed';
    dbInsert(array('user' => $_SESSION['username'], 'address' => get_client_ip(), 'result' => $auth_error), 'authlog');
    unset($_SESSION['authenticated']); // make sure this isn't set if there was an error
}

session_write_close();
unset($auth_error, $username, $password);
