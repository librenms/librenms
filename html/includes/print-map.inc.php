...
foreach ($list as $items) {
    ...
    if (!array_key_exists($local_device_id, $devices_by_id)) {
        ...
        $devices_by_id[$local_device_id] = array('id'=>$local_device_id,'label'=>shorthost(process_hostname($items, $items['local_hostname']), 1),'title'=>generate_device_link($local_device, '', array(), '', '', '', 0),'shape'=>'box');
    }
    ...
    if (!array_key_exists($remote_device_id, $devices_by_id)) {
        ...
        $devices_by_id[$remote_device_id] = array('id'=>$remote_device_id,'label'=>shorthost(process_hostname($items, $items['remote_hostname']), 1),'title'=>generate_device_link($remote_device, '', array(), '', '', '', 0),'shape'=>'box');
    }
