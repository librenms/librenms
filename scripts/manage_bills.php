...
function add_ports_to_bill($devs, $intf_glob, $id)
{
    ...
    $query = "SELECT ports.port_id,ports.ifName,ports.ifAlias FROM ports INNER JOIN devices ON ports.device_id = devices.device_id WHERE ifType != 'softwareLoopback' AND ports.ifAlias LIKE '%$intf_glob%' AND ports.device_id in ($ids)";
    echo("Query: $query\n");
