...
class BgpBackwardTransition implements SnmptrapHandler
{
    ...
    public function handle(Device $device, Trap $trap)
    {
        ...
        if ($bgpPeer->isDirty('bgpPeerState')) {
            \Log::event('SNMP Trap: BGP Down ' . $bgpPeer->bgpPeerIdentifier . ' ' . get_astext($bgpPeer->bgpPeerRemoteAs) . ' is now ' . $bgpPeer->bgpPeerState, $device->device_id, 'bgpPeer', 5, $bgpPeer->bgpPeer_id);
        }
