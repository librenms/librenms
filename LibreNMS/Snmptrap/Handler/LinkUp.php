...
class LinkUp implements SnmptrapHandler
{
    ...
    public function handle(Device $device, Trap $trap)
    {
        ...
        log_event("SNMP Trap: linkUp $port->ifAdminStatus/$port->ifOperStatus " . $port['ifDescr'], $device, "interface", 1, $port['port_id']);

        if ($port->isDirty('ifAdminStatus')) {
            log_event("Interface Enabled : " . $port['ifDescr'] . " (TRAP)", $device, "interface", 3, $port['port_id']);
        }

        if ($port->isDirty('ifOperStatus')) {
            log_event("Interface went Up : " . $port['ifDescr'] . " (TRAP)", $device, "interface", 1, $port['port_id']);
        }
