    ...
    WirelessApCountDiscovery
{
    ...
    public function pollOS(DataStorageInterface $datastore): void
    {
        ...
        $apNames = \SnmpQuery::enumStrings()->walk('CISCO-LWAPP-AP-MIB::cLApName')->table(1);
        $radios = \SnmpQuery::enumStrings()->walk('CISCO-LWAPP-AP-MIB::cLApDot11IfType')->table(2);
        ...
        foreach ($radios as $mac => $radio) {
            foreach ($radio as $slot => $value) {
                $channel = str_replace('ch', '', $value['CISCO-LWAPP-AP-MIB::cLApDot11IfPhyS80EightyChannelNumber'] ?? '');

                $ap = new AccessPoint([
                    'device_id' => $this->getDeviceId(),
                    'name' => $apNames[$mac]['CISCO-LWAPP-AP-MIB::cLApName'] ?? '',
                    'radio_number' => $slot,
                    'type' => $value['CISCO-LWAPP-AP-MIB::cLApDot11IfType'] ?? '',
                    'mac_addr' => $mac,
                    'channel' => $channel,
