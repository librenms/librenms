    ...
    WirelessRateDiscovery
{
    ...
    public function discoverWirelessFrequency()
    {
        ...
        return [
            new WirelessSensor(WirelessSensorType::Frequency, $this->getDeviceId(), $oid, 'airos-af-ltu', 1, 'Radio Frequency'),
        ];
    }
    ...
    public function discoverWirelessDistance()
    {
        ...
        foreach ($oids as $index => $entry) {
            return [
                new WirelessSensor(WirelessSensorType::Distance, $this->getDeviceId(), '.1.3.6.1.4.1.41112.1.10.1.4.1.23.' . $index, 'airos-af-ltu', 1, 'Distance', $entry['afLTUStaRemoteDistance'], 1, 1000), //UBNT-AFLTU-MIB::afLTUStaRemoteDistance
            ];
        }
    ...
    public function discoverWirelessPower()
    {
        ...
        $sensors[] = new WirelessSensor(WirelessSensorType::Power, $this->getDeviceId(), $tx_eirp_oid, 'airos-af-ltu-tx-eirp', 1, 'TX EIRP');
    ...
    public function discoverWirelessQuality()
    {
        ...
        foreach ($oids as $index => $entry) {
            $sensors[] = new WirelessSensor(WirelessSensorType::Quality, $this->getDeviceId(), '.1.3.6.1.4.1.41112.1.10.1.4.1.9.' . $index, 'airos-af-ltu-level-rx-chain-0', 1, 'Signal Level Chain 0', $entry['afLTUStaRxPowerLevel0']); //UBNT-AFLTU-MIB::afLTUStaRxPowerLevel0
            $sensors[] = new WirelessSensor(WirelessSensorType::Quality, $this->getDeviceId(), '.1.3.6.1.4.1.41112.1.10.1.4.1.10.' . $index, 'airos-af-ltu-level-rx-chain-1', 1, 'Signal Level Chain 1', $entry['afLTUStaRxPowerLevel1']); //UBNT-AFLTU-MIB::afLTUStaRxPowerLevel1
            break;
        }
