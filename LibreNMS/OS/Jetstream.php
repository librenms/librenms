...
class Jetstream extends OS implements Ipv6AddressDiscovery, RouteDiscovery, FdbTableDiscovery
{
    ...
    public function discoverFdbTable(): Collection
    {
        ...
        $tpFdbPort = \SnmpQuery::allowUnordered()->hideMib()->walk('Q-BRIDGE-MIB::dot1qTpFdbPort')->table();
        $tpFdbPort = (! empty($tpFdbPort)) ? array_shift($tpFdbPort) : [];
        Log::info('Jetstream FDB Table');
        foreach ($tpFdbPort as $realVlan => $macData) {
