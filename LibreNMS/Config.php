...
class Config
{
    ...
    public static function getExecutable(string $key): string
    {
        ...
        if (Eloquent::isConnected()) {
            ...
            $notification->title = 'Missing executable';
            $notification->body = "The executable: $binary, cannot be found in your \$PATH, please make sure you install it!";
            $notification->severity = 1;
            $notification->source = 'Config';
            $notification->datetime = date('Y-m-d H:i:s');
            $notification->checksum = hash('sha512', $notification->title . $notification->body);

            if ($existing = Notification::where('checksum', $notification->checksum)->first()) {
                $existing->markRead(false);
            } else {
                $notification->save();
            }
        }
