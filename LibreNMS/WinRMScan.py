...
def process_polling(self, device_id):
    if isInstall == True:
        ...
        session = winrm.Session(
            hostname,
            auth=(self.config.winrm.username, self.config.winrm.password),
            transport="ntlm",
        )
        ...
        return (
            -1,
            "Unable to load WinRM library. Please install pywinrm using - pip install pywinrm.",
        )
...
def get_device_details(self, device_id):
    devices = self._db.query(
        "SELECT `hostname` FROM `devices` WHERE (`device_id` = %s)", (device_id)
    )
    ...
    if isInstall == True:
        try:
            ...
            result = session.run_ps(
                "Get-Service | Select-Object ServiceName,DisplayName,Status,ServiceType,StartType,CanPauseAndContinue,CanShutdown,CanStop | ConvertTo-Json"
            )
            ...
            if result.status_code == 0:
                ...
                for service in services:
                    try:
                        self._db.query(
                            "INSERT INTO `winrm_services` (`device_id`, `service_name`, `display_name`, `status`, `service_type`, `start_type`, `can_pause_and_continue`, `can_shutdown`, `can_stop`, `disabled`) "
...
def process_processes(self, session, device_id):
    if isInstall == True:
        try:
            # self.config.winrm.username
            result = session.run_ps(
                "Get-Process -IncludeUserName | Select-Object Id,Name,ProcessName,UserName,NPM,PM,WS,VM,CPU | ConvertTo-Json"
            )
            ...
            if result.status_code == 0:
                ...
                for processe in processes:
                    try:
                        ...
                        self._db.query(
                            "INSERT INTO `winrm_processes` (`device_id`, `pid`, `name`, `process_name`, `username`, `npm`, `pm`, `ws`, `vm`, `cpu`) "
...
def process_software(self, session, device_id):
    if isInstall == True:
        try:
            result = session.run_ps(
                "Get-WmiObject -Class Win32_Product | Select-Object Name,Vendor,Version,InstallDate,Description | ConvertTo-Json"
            )
            if result.status_code == 0:
                ...
                for application in software:
                    try:
                        if application["Name"] is not None:
                            self._db.query(
                                "INSERT INTO `winrm_software` (`name`, `vendor`, `description`) "
                            ...
                            application_id = self._db.query(
                                "SELECT `id` FROM `winrm_software` "
                            ...
                            if application["InstallDate"] is not None:
                                install_date = datetime.strptime(
                                    application["InstallDate"], "%Y%m%d"
                                ).strftime("%Y/%m/%d")

                            for software_id in application_id:
                                self._db.query(
                                    "INSERT INTO `winrm_device_software` (`device_id`, `software_id`, `version`, `install_date`, `disabled`) "
