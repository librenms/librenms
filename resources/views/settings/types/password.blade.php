<div class="form-group has-feedback {{ $class }}">
    ...
    <div class="col-sm-6 col-lg-4">
        <input id="{{ $name }}" class="form-control validation" type="password" name="{{ $name }}" value="{{ $value }}"
               data-config_id="{{ $name }}" @if($pattern)pattern="{{ $pattern }}"@endif @if($required) required @endif>
