    ...
    @if($data['smokeping']->hasGraphs())
        <div class="panel with-nav-tabs panel-default">
            <div class="panel-heading">
                <a href="{{ \LibreNMS\Config::get('smokeping.url') }}?target={{ $device->type }}.@if(\LibreNMS\Config::get('smokeping.use_folders') === true)@if(preg_match('/^([0-9]{1,3}.){3}[0-9]{1,3}$/',$device->hostname)){{ preg_replace('/_[^_]+_[^_]+$/','.',str_replace('.','_',$device->hostname)) }}{{ preg_replace('/_[^_]+$/','.',str_replace('.','_',$device->hostname)) }}@endif{{ str_replace('.','_',$device->hostname) }}.@endif{{ str_replace('.','_',$device->hostname) }}" target="_blank"><span class="panel-title">@lang('Smokeping') <i class="glyphicon glyphicon-share-alt"></i></span></a>
