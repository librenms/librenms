<x-option-bar name="{{ __('BGP') }} ({{ __('Local AS') }}: {{ $data['local_as'] ?? __('N/A') }})" :options="$data['bgp_options']" :selected="$data['view']" />
    ...
    <div class="table-responsive">
        <table class="table table-hover table-condensed table-striped">
            ...
            @forelse($data['peers'] as $peerData)
                <tr>
                    <td>
                        <a href="{{ route('device', ['device' => $device, 'tab' => 'routing', 'proto' => 'bgp', 'view' => 'updates']) }}" class="tw:font-bold">
                            {{ $peerData['identifier_compressed'] }}
                        </a>
                        @if($peerData['linked_port'])
                            <br>
                            <x-device-link :device="$peerData['linked_port']->device" tab="routing" proto="bgp" />
