@props(['device', 'application'])

@if($application)
    @php($metrics = $application->metrics->pluck('value', 'metric'))
    <x-device.overview.panel :title="__('Puppet Agent')" icon="fa fa-cogs"
        ...
        <div class="tw:divide-y tw:divide-gray-300 tw:dark:divide-zinc-800">
            <div class="tw:grid tw:grid-cols-3 tw:gap-3 tw:p-3 tw:text-center">
                @foreach([
            ...
            <div class="tw:grid tw:grid-cols-3 tw:gap-3 tw:p-3 tw:text-center">
                ...
                <span>{{ __('Total') }}: {{ $metrics['events_total'] ?? 0 }}</span>
            </div>
        </div>
