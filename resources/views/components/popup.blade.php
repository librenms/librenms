<div {{ $attributes->except('target-class')->merge(['class' => 'tw:inline-block tw:relative']) }}
...
>
    <span class="{{ $attributes->get('target-class') }}" x-ref="targetRef" x-on:mouseenter='show(showDelay)' x-on:mouseleave="hide(hideDelay)">
        {{ $slot }}
    </span>
    <div x-cloak
          x-ref="popupRef"
          ...
          class="tw:hidden tw:absolute tw:max-h-[calc(100vh-1rem)] tw:max-w-[calc(100vw-1rem)] tw:overflow-auto tw:bg-white! tw:dark:bg-dark-gray-300! tw:dark:text-white tw:border-2 tw:border-gray-200 tw:dark:border-dark-gray-200 tw:z-9999 tw:font-normal tw:leading-normal tw:text-sm tw:text-left tw:no-underline tw:rounded-lg"
    >
        @isset($title)
