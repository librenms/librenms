<div
    {{ $attributes->merge(['class' => 'tw:relative tw:inline-flex tw:items-center tw:max-w-full tw:min-w-0' . ($hide ? ' tw:hidden' : '')]) }}
    ...
    {{-- Main Bar --}}
<div class="tw:group/bar tw:flex tw:flex-col tw:sm:flex-row tw:flex-nowrap tw:max-w-full tw:min-w-0" x-show="!hide">
    <div class="tw:flex tw:items-stretch tw:flex-col tw:sm:flex-row tw:h-auto tw:sm:h-10 tw:rounded-lg tw:border tw:border-neutral-300 tw:dark:border-dark-gray-300 tw:bg-white tw:dark:bg-dark-gray-500 tw:font-mono tw:shadow-xs tw:max-w-full tw:min-w-0">

        {{-- LEFT SECTION: Options Dropdown --}}
        <div class="tw:relative tw:flex tw:items-stretch tw:shrink-0 tw:max-sm:w-full">
            <button type="button"
                    :title="filters.length ? '{{ __('Filter options') }}' : '{{ __('Open filter menu') }}'"
                    @click.stop="toggleOptions()"
                    class="tw:sm:shrink-0 tw:w-full tw:sm:w-auto tw:flex tw:items-center tw:gap-2.5 tw:py-2 tw:sm:py-0 tw:px-4 tw:h-full tw:transition-colors tw:border-b tw:sm:border-b-0 tw:sm:border-r tw:border-neutral-200 tw:dark:border-dark-gray-300 tw:rounded-tl-lg tw:rounded-tr-lg tw:sm:rounded-tr-none tw:sm:rounded-l-lg tw:hover:bg-neutral-50 tw:dark:hover:bg-dark-gray-400">
                <div class="tw:relative tw:flex tw:items-center">
                    <i class="fa-solid fa-filter tw:text-base"></i>
                ...
                <span class="tw:font-bold" :class="filters.length ? 'tw:inline tw:sm:hidden' : ''">{{ __('Filter') }}</span>
                <span class="tw:font-semibold tw:ml-auto" x-show="filters.length" x-text="filters.length"></span>
            </button>
        ...
        <button type="button" x-show="canScrollLeft" x-cloak
                ...
                class="tw:hidden tw:sm:flex tw:items-center tw:justify-center tw:w-7 tw:h-full tw:text-neutral-400! tw:hover:text-neutral-900! tw:dark:text-dark-white-400! tw:dark:hover:text-dark-white-100! tw:hover:bg-neutral-100 tw:dark:hover:bg-dark-gray-400 tw:transition-colors tw:shrink-0 tw:border-r tw:border-neutral-200 tw:dark:border-dark-gray-300">
            <i class="fa-solid fa-chevron-left tw:text-xs"></i>
        </button>
        ...
        <div x-ref="chips"
             ...
             class="tw:flex tw:flex-col tw:sm:flex-row tw:items-stretch tw:overflow-x-auto tw:scrollbar-none tw:flex-nowrap tw:min-w-0"
             @wheel.passive="if ($el.scrollWidth > $el.clientWidth && $event.deltaY) { $el.scrollLeft += $event.deltaY; updateScroll(); }">
            <template x-for="f in filters" :key="f.key">
                <div class="filter-chip tw:shrink-0 tw:relative tw:group tw:flex tw:items-stretch tw:h-full tw:border-b tw:sm:border-b-0 tw:sm:border-r tw:border-neutral-200 tw:dark:border-dark-gray-300"
                    ...
                    <button type="button" title="{{ __('Edit filter') }}"
                            ...
                            class="tw:flex tw:items-center tw:py-2 tw:sm:py-0 tw:px-4 tw:gap-2.5 tw:transition-colors tw:hover:bg-neutral-50 tw:dark:hover:bg-dark-gray-400 tw:whitespace-nowrap">
                    ...
                    <button type="button" title="{{ __('Remove filter') }}" @click.stop="remove(f.key)"
                            class="tw:h-full tw:w-0 tw:py-2 tw:sm:py-0 tw:group-hover:w-10 tw:flex tw:items-center tw:justify-center tw:bg-neutral-100 tw:dark:bg-dark-gray-400 tw:text-neutral-500! tw:dark:text-dark-white-400! tw:transition-all tw:duration-200 tw:ease-in-out tw:overflow-hidden tw:text-base tw:hover:text-red-600! tw:dark:hover:text-red-400!">
        ...
        <button type="button" x-show="canScrollRight" x-cloak
                ...
                class="tw:hidden tw:sm:flex tw:items-center tw:justify-center tw:w-7 tw:h-full tw:text-neutral-400! tw:hover:text-neutral-900! tw:dark:text-dark-white-400! tw:dark:hover:text-dark-white-100! tw:hover:bg-neutral-100 tw:dark:hover:bg-dark-gray-400 tw:transition-colors tw:shrink-0 tw:border-r tw:border-neutral-200 tw:dark:border-dark-gray-300">
            <i class="fa-solid fa-chevron-right tw:text-xs"></i>
        </button>
        ...
        <div class="tw:shrink-0 tw:max-sm:left-0 tw:flex tw:flex-col tw:sm:flex-row tw:items-stretch">
            ...
            <button type="button"
                    ...
                    class="tw:w-10 tw:py-2 tw:sm:py-0 tw:h-full tw:relative tw:max-sm:w-full tw:flex tw:items-center tw:justify-center tw:text-base tw:text-neutral-400! tw:dark:text-dark-white-400! tw:hover:text-neutral-900! tw:dark:hover:text-dark-white-100! tw:hover:bg-neutral-50 tw:dark:hover:bg-dark-gray-400 tw:transition-colors tw:border-b tw:sm:border-b-0 tw:sm:border-r tw:border-neutral-200 tw:dark:border-dark-gray-300">
            ...
            <div x-show="showAdd" x-cloak x-transition @click.stop
                 ...
                 class="tw:absolute tw:top-full tw:mt-2 tw:w-max tw:min-w-44 tw:max-w-[90vw] tw:bg-white tw:dark:bg-dark-gray-400 tw:border tw:border-neutral-200 tw:dark:border-dark-gray-200 tw:rounded-lg tw:shadow-xl tw:z-50 tw:py-1.5">
                ...
                @foreach($fields as $index => $field)
                    <button type="button" @click="open(fields[{{ $index }}])"
                            ...
                            class="tw:flex tw:items-center tw:w-full tw:px-4 tw:py-2.5 tw:whitespace-nowrap tw:text-left tw:hover:bg-neutral-50 tw:dark:hover:bg-dark-gray-300 tw:transition-colors tw:gap-3">
    ...
    <div x-show="filters.length > 0"
         class="tw:hidden tw:sm:block tw:transition-all tw:duration-200 tw:ease-in-out tw:overflow-hidden tw:group-has-[.filter-chip:hover]/bar:w-0">
    </div>
</div>
    ...
    <template x-teleport="body">
        <div x-show="dialog" x-cloak
            ...
            <div x-show="dialog" x-transition @click.stop
                ...
                {{-- Header --}}
                <div class="tw:px-6 tw:py-4 tw:border-b tw:border-neutral-100 tw:dark:border-dark-gray-300 tw:bg-blue-50/20 tw:dark:bg-dark-gray-400 tw:flex tw:items-center tw:justify-between tw:rounded-t-2xl">
                    <div class="tw:text-base tw:font-bold tw:text-neutral-900! tw:dark:text-dark-white-100!">
                        <span x-text="current?.label"></span>
                        <span class="tw:ml-1.5 tw:text-xs tw:font-semibold tw:text-blue-600! tw:dark:text-blue-400! tw:uppercase">{{ __('Filter') }}</span>
                    </div>
                    <button type="button" title="{{ __('Close dialog') }}" @click="close()"
                            class="tw:text-neutral-400! tw:dark:text-dark-white-400! tw:transition-colors tw:hover:text-red-500! tw:dark:hover:text-red-500! tw:text-lg">
                        <i class="fa-solid fa-xmark"></i>
                    </button>
                </div>
                ...
                <div class="tw:p-6 tw:space-y-6">
                    <div>
                        <span class="tw:block tw:text-xs tw:font-bold tw:text-neutral-500 tw:dark:text-dark-white-400 tw:uppercase tw:tracking-wider tw:mb-2.5">{{ __('Condition') }}</span>
                        <div class="tw:grid tw:grid-cols-2 tw:gap-2">
                            <template x-for="o in ops()" :key="o.v">
                                <button type="button" @click="op = o.v"
                                        class="tw:flex tw:items-center tw:gap-3 tw:px-3.5 tw:py-2.5 tw:rounded-lg tw:text-sm tw:font-bold tw:transition-all tw:border"
                                        :class="op === o.v
                                            ? 'tw:bg-blue-600 tw:dark:bg-blue-600 tw:text-white! tw:dark:text-white! tw:border-blue-600 tw:shadow-xs'
                                    ...
                                    <span class="tw:w-4 tw:text-center tw:opacity-70" x-text="o.s"></span>
                    ...
                    <div x-show="!isNullary()">
                        <span class="tw:block tw:text-xs tw:font-bold tw:text-neutral-500 tw:dark:text-dark-white-400 tw:uppercase tw:tracking-wider tw:mb-2.5">{{ __('Value') }}</span>
                            ...
                            <div class="tw:flex tw:flex-col tw:gap-2">
                                <template x-for="opt in getNormalizedOptions()" :key="opt.value">
                                    <button type="button"
                                            ...
                                            class="tw:flex tw:items-center tw:justify-between tw:w-full tw:px-3.5 tw:py-2.5 tw:rounded-lg tw:text-sm tw:font-bold tw:transition-all tw:border tw:text-left"
                                            :class="(isMulti() ? value.includes(opt.value) : value === opt.value)
                                              ? 'tw:bg-blue-600 tw:text-white! tw:border-blue-600 tw:shadow-xs'
                                              : 'tw:bg-neutral-50 tw:dark:bg-dark-gray-400 tw:text-neutral-600! tw:dark:text-dark-white-200! tw:border-neutral-100 tw:dark:border-dark-gray-300 tw:hover:bg-neutral-100 tw:dark:hover:bg-dark-gray-300'">
                        ...
                        <template x-if="current && current.type === 'boolean'">
                            <div class="tw:grid tw:grid-cols-2 tw:gap-3">
                                <button type="button" @click="setBoolean(1, 'Yes')"
                                        class="tw:py-2.5 tw:rounded-lg tw:font-bold tw:text-sm tw:border tw:transition-all"
                                        :class="value == 1
                ...
                {{-- Footer --}}
                <div class="tw:flex tw:items-center tw:justify-between tw:px-6 tw:py-4 tw:bg-neutral-50/50 tw:dark:bg-dark-gray-400 tw:rounded-b-2xl tw:border-t tw:border-neutral-100 tw:dark:border-dark-gray-300">
                    <button type="button" x-show="isActive(current?.key)" @click="remove(current.key)"
                            class="tw:h-9 tw:px-4 tw:bg-white tw:dark:bg-dark-gray-500 tw:text-neutral-600! tw:dark:text-red-400! tw:border tw:border-neutral-200 tw:dark:border-dark-gray-200 tw:rounded-lg tw:text-sm tw:font-bold tw:transition-colors tw:hover:bg-red-50 tw:dark:hover:bg-red-900/20">
                        {{ __('Remove') }}
                    </button>
                    <div x-show="!isActive(current?.key)"></div>
                    <div class="tw:flex tw:gap-3">
                        <button type="button" @click="close()"
                                class="tw:h-9 tw:px-4 tw:bg-white tw:dark:bg-dark-gray-500 tw:border tw:border-neutral-200 tw:dark:border-dark-gray-300 tw:text-sm tw:font-bold tw:text-neutral-600! tw:dark:text-dark-white-300! tw:rounded-lg tw:transition-colors tw:hover:bg-neutral-50 tw:dark:hover:bg-dark-gray-400">
                            {{ __('Cancel') }}
                        </button>
                        <button type="button" @click="apply()"
                                :disabled="!isNullary() && (isMulti() ? !value.length : value === '' || value === null)"
                                class="tw:h-9 tw:px-5 tw:text-sm tw:font-bold tw:bg-blue-600 tw:text-white! tw:rounded-lg tw:transition-all tw:hover:bg-blue-700 tw:dark:hover:bg-blue-500 tw:disabled:opacity-20">
                            {{ __('Apply') }}
                        </button>
                    </div>
                </div>
