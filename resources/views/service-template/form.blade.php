    ...
    <div class="col-sm-9 col-md-10">
        <select class="form-control" id="device_group_id" name="device_group_id">
            @foreach($device_groups as $device_group)
                <option value="{{ $device_group->id }}" @if($device_group->id == $template->device_group_id) selected @endif>{{ $device_group->name }}</option>
            @endforeach
        </select>
...
<div class="form-group @if($errors->has('ignore')) has-error @endif">
    ...
    <div class="col-sm-9 col-md-10">
        <input type="checkbox" class="form-control" id="ignore" name="ignore" data-size="small" value="{{ old('disabled', $template->disabled) }}" @if(old('ignore', $template->ignore)) checked @endif>
...
<div class="form-group @if($errors->has('ip')) has-error @endif">
    ...
    <div class="col-sm-9 col-md-10">
        <input type="checkbox" class="form-control" id="disabled" name="disabled" data-size="small" value="{{ old('disabled', $template->disabled) }}"@if(old('disabled', $template->disabled)) checked @endif>
