...
class CreatePortGroupsTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('port_group_port', function (Blueprint $table) {
            ...
            $table->foreign('port_group_id')->references('id')->on('port_groups')->onUpdate('RESTRICT')->onDelete('CASCADE');
            $table->foreign('port_id')->references('port_id')->on('ports')->onUpdate('RESTRICT')->onDelete('CASCADE');
        });
    }
