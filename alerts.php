...
function RunFollowUp() {
	...
	foreach( dbFetchRows("SELECT alerts.device_id, alerts.rule_id, alerts.state FROM alerts,alert_rules WHERE alerts.rule_id = alert_rules.id && alert_rules.disabled = 0 && alerts.state != 2 && alerts.open = 0") as $alert ) {
		...
		$alert = dbFetchRow("SELECT alert_log.id,alert_log.rule_id,alert_log.device_id,alert_log.state,alert_log.details,alert_log.time_logged,alert_rules.rule,alert_rules.severity,alert_rules.extra,alert_rules.name,alert_rules.disabled FROM alert_log,alert_rules WHERE alert_log.rule_id = alert_rules.id && alert_log.device_id = ? && alert_log.rule_id = ? ORDER BY alert_log.id DESC LIMIT 1",array($alert['device_id'],$alert['rule_id']));
		if( empty($alert['rule']) ) {
		...
		if( $alert['disabled'] == 1){
                        echo "Rule ".$alert['rule_id']." is Disabled for Alert-UID #".$alert['id']."\r\n";
                        continue;
                }
...
function RunAlerts() {
	...
	foreach( dbFetchRows("SELECT alerts.device_id, alerts.rule_id, alerts.state FROM alerts,alert_rules WHERE alerts.rule_id = alert_rules.id && alert_rules.disabled = 0 && alerts.state != 2 && alerts.open = 1") as $alert ) {
		...
		if( $updet ) {
			dbUpdate(array('details' => gzcompress(json_encode($alert['details']),9)),'alert_log','id = '.$alert['id']);
		}
