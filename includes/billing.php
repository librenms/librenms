...
{
    $ptot = dbFetchRow('SELECT SUM(period) as `period`, MAX(in_delta) as `max_in`, MAX(out_delta) as `max_out`  FROM bill_data WHERE bill_id = ? AND timestamp > ? AND timestamp <= ?', [$bill_id, $datefrom, $dateto]);
