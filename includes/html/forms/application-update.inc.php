...
if (! Auth::user()->hasGlobalAdmin()) {
    ...
    if (! isset($app) && $device === null) {
        ...
        $app = Application::withTrashed()->firstOrNew(['device_id' => $device->device_id, 'app_type' => $app]);
        if ($_POST['state'] == 'true') {
            if ($app->trashed()) {
                $app->restore();
            }
            if ($app->save()) {
                log_event("Application enabled by user: $app", $device->device_id, 'application', 1);
            ...
            $app->delete();
            if ($app->save()) {
                log_event("Application disabled by user: $app", $device->device_id, 'application', 3);
