...
if (! empty($ifName) && is_numeric($port_id)) {
    ...
    if (dbUpdate(['ifAlias' => $descr], 'ports', '`port_id`=?', [$port_id]) > 0) {
        ...
        if ($descr === 'repoll') {
            ...
            Eventlog::log("$ifName Port ifAlias cleared manually", $device, 'interface', 3, $port_id);
        } else {
            set_dev_attrib($device, 'ifName:' . $ifName, 1);
            Eventlog::log("$ifName Port ifAlias set manually: $descr", $device, 'interface', 3, $port_id);
        }
        $status = 'ok';
    } else {
        $status = 'na';
