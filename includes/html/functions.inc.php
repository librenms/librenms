 ...
 * @return string
 *
 * .headers :: A array of header items
 * .rows :: A array of arrays. First level is row and second is column.
 */
function render_table($table_info)
{
    ...
    if (isset($table_info['rows']) && is_array($table_info['rows']) && isset($table_info['rows'][0])) {
        foreach ($table_info['rows'] as $row) {
            $to_return = $to_return . "  <tr>\n";
