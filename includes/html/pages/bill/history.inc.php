...
foreach (dbFetchRows('SELECT * FROM `bill_history` WHERE `bill_id` = ? ORDER BY `bill_datefrom` DESC LIMIT 24', [$bill_id]) as $history) {
    if (bill_permitted($history['bill_id'])) {
        ...
        $total_data = Number::formatBase($history['traf_total'], \App\Facades\Config::get('billing.base'), 2, 3, '');
        ...
        if ($type == 'CDR') {
            ...
            $allowed = Number::formatBase($history['bill_allowed'], \App\Facades\Config::get('billing.base'), 2, 3, '');
            $used = Number::formatBase($history['total_data'], \App\Facades\Config::get('billing.base'), 2, 3, '');
            $in = Number::formatBase($history['traf_in'], \App\Facades\Config::get('billing.base'), 2, 3, '');
            $out = Number::formatBase($history['traf_out'], \App\Facades\Config::get('billing.base'), 2, 3, '');
            $overuse = (($history['bill_overuse'] <= 0) ? '-' : '<span style="color: #' . $background['left'] . '; font-weight: bold;">' . Number::formatBase($history['bill_overuse'], \App\Facades\Config::get('billing.base')) . '</span>');
        }
        $peakOut = Number::formatBase($history['bill_peak_out'], \App\Facades\Config::get('billing.base'), 2, 3, '');
        $peakIn = Number::formatBase($history['bill_peak_in'], \App\Facades\Config::get('billing.base'), 2, 3, '');
