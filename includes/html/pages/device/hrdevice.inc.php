...
foreach (dbFetchRows('SELECT * FROM `hrDevice` WHERE `device_id` = ? ORDER BY `hrDeviceIndex`', [$device['device_id']]) as $hrdevice) {
    ...
    if ($hrdevice['hrDeviceType'] == 'hrDeviceProcessor') {
        ...
        $proc_popup .= "</div><img src=\'graph.php?id=" . $proc_id . '&amp;type=processor_usage&amp;from=' . \App\Facades\Config::get('time.month') . '&amp;to=' . \App\Facades\Config::get('time.now') . "&amp;width=400&amp;height=125\'>";
        $proc_popup .= "', RIGHT" . \App\Facades\Config::get('overlib_defaults') . ');" onmouseout="return nd();"';
