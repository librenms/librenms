...
$option_default = $vars['topN'] ?? \App\Facades\Config::get('nfsen_top_default');

$option_int = 0;
foreach (\App\Facades\Config::get('nfsen_top_N') as $option) {
...
$option_keys = array_keys(\App\Facades\Config::get('nfsen_lasts'));
$options = \App\Facades\Config::get('nfsen_lasts');
...
$option_default = $vars['stattype'] ?? \App\Facades\Config::get('nfsen_stats_default');
...
$option_default = \App\Facades\Config::get('nfsen_order_default');
...
if (isset($vars['process'])) {
    ...
    if (isset($vars['lastN']) &&
         ...
         ($vars['lastN'] <= \App\Facades\Config::get('nfsen_last_max'))
        ) {
        $lastN = $vars['lastN'];
    }
    ...
    if (isset($vars['topN']) &&
         ...
         ($vars['topN'] <= \App\Facades\Config::get('nfsen_top_max'))
        ) {
        $topN = $vars['topN'];
    }
    ...
    $command = \App\Facades\Config::get('nfdump') . ' -M ' . nfsen_live_dir($device['hostname']) . ' -T -R ' .
