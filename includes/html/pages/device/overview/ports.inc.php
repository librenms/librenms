...
if (ObjectCache::portCounts(['total'], $device['device_id'])['total'] > 0) {
    ...
    <a class="lnms-btn lnms-btn-success" role="button" href="' . route('device', ['device' => $device['device_id'], 'tab' => 'ports', 'filter' => ['state' => ['eq' => 'up']]]) . '">Up: <span class="lnms-btn-badge">' . $ports['up'] . '</span></a>
    <a class="lnms-btn lnms-btn-danger" role="button" href="' . route('device', ['device' => $device['device_id'], 'tab' => 'ports', 'filter' => ['state' => ['eq' => 'down']]]) . '">Down: <span class="lnms-btn-badge">' . $ports['down'] . '</span></a>
