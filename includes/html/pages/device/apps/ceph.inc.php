...
foreach ($graphs as $key => $text) {
    ...
    if ($key == 'ceph_poolstats') {
        foreach (Rrd::getRrdFiles($device['hostname'], ['app', 'ceph', $app['app_id'], 'pool']) as $rrd_filename) {
        ...
        foreach (Rrd::getRrdFiles($device['hostname'], ['app', 'ceph', $app['app_id'], 'osd']) as $rrd_filename) {
            $graph_array['to'] = \App\Facades\LibrenmsConfig::get('time.now');
            $graph_array['id'] = $app['app_id'];
        ...
        foreach (Rrd::getRrdFiles($device['hostname'], ['app', 'ceph', $app['app_id'], 'df']) as $rrd_filename) {
