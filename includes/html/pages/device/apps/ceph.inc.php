...
foreach ($graphs as $key => $text) {
    ...
    if ($key == 'ceph_poolstats') {
        foreach (glob(Rrd::partname($device['hostname'], ['app', 'ceph', $app['app_id'], 'pool'], true) . '-*.rrd') as $rrd_filename) {
        ...
        foreach (glob(Rrd::partname($device['hostname'], ['app', 'ceph', $app['app_id'], 'osd'], true) . '-*.rrd') as $rrd_filename) {
            $graph_array['to'] = \App\Facades\LibrenmsConfig::get('time.now');
            $graph_array['id'] = $app['app_id'];
        ...
        foreach (glob(Rrd::partname($device['hostname'], ['app', 'ceph', $app['app_id'], 'df'], true) . '-*.rrd') as $rrd_filename) {
