...
foreach ($graphs as $key => $text) {
    ...
    if ($key == 'ceph_poolstats') {
        foreach (Rrd::globnames($device['hostname'], ['app', 'ceph', $app['app_id'], 'pool'], '-*.rrd') as $rrd_filename) {
        ...
        foreach (Rrd::globnames($device['hostname'], ['app', 'ceph', $app['app_id'], 'osd'], '-*.rrd') as $rrd_filename) {
            $graph_array['to'] = \App\Facades\LibrenmsConfig::get('time.now');
            $graph_array['id'] = $app['app_id'];
        ...
        foreach (Rrd::globnames($device['hostname'], ['app', 'ceph', $app['app_id'], 'df'], '-*.rrd') as $rrd_filename) {
