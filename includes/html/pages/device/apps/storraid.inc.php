...
foreach ($controllers as $ctrl) {
    if (isset($ctrl['error'])) {
        echo "<div class='alert alert-danger'>Controller error: " . htmlspecialchars($ctrl['error']) . "</div>";
        continue;
    }
    ...
    if ($bbu) {
        ...
        <tr>
          ...
          <td>{$bbu['type']} &nbsp; {$bbu_badge} &nbsp; State: <strong>"
            ...
            . ($bbu['temperature'] !== 'N/A' ? " &nbsp; Temp: " . htmlspecialchars($bbu['temperature']) : '')
            . ($bbu['charge_pct']  !== 'N/A' ? " &nbsp; Charge: " . htmlspecialchars($bbu['charge_pct']) : '')
            . "</td>
        </tr>";
    }
    echo "
    ...
    <div class='panel panel-default'>
      <div class='panel-heading'>
        ...
        . htmlspecialchars($ctrl['model']) . " &nbsp; {$badge}
      </div>
      <div class='panel-body' style='padding:0'>
        <table class='table table-condensed' style='table-layout:fixed;width:100%;margin-bottom:0'>
          ...
          <tr><th>Serial</th><td>"      . htmlspecialchars($ctrl['serial'])      . "</td></tr>
          <tr><th>Firmware</th><td>"    . htmlspecialchars($ctrl['firmware'])    . "</td></tr>
          <tr><th>Memory</th><td>"      . htmlspecialchars($ctrl['memory_mb'])   . "</td></tr>
...
foreach ($virtual_disks as $vd) {
    ...
    $prog  = ($vd['progress_pct'] !== null)
        ...
        : '&mdash;';
    $row_class = match ($sev) { 0 => '', 1 => 'warning', default => 'danger' };

    echo "
    <tr class='{$row_class}'>
      <td>{$icon}</td>
      <td>c{$vd['controller']}</td>
      <td>" . htmlspecialchars($vd['id'])         . "</td>
      <td>" . htmlspecialchars($vd['name'])        . "</td>
      <td><strong>" . htmlspecialchars($vd['raid_level']) . "</strong></td>
      <td>" . htmlspecialchars($vd['size'])        . "</td>
      <td><strong>{$state}</strong></td>
      <td>" . htmlspecialchars($vd['access'])      . "</td>
      <td>" . htmlspecialchars($vd['cache'])       . "</td>
      <td>" . htmlspecialchars($vd['consist'])     . "</td>
      <td>{$prog}</td>
    </tr>";
}
echo "</tbody></table>";
...
foreach ($physical_disks as $pd) {
    ...
    <tr class='{$row_class}'>
      ...
      <td>" . htmlspecialchars($pd['eid_slot'])     . "</td>
      <td>" . htmlspecialchars($pd['vd'])           . "</td>
      <td>{$model_link}</td>
      <td>" . htmlspecialchars($pd['serial'] ?? 'N/A') . "</td>
      <td>" . htmlspecialchars(trim($pd['firmware'] ?? 'N/A')) . "</td>
      <td>" . htmlspecialchars($pd['size'])         . "</td>
      <td>" . htmlspecialchars($pd['media_type'])   . "</td>
      <td>" . htmlspecialchars($pd['interface'])    . "</td>
      <td><strong>" . htmlspecialchars($pd['state']) . "</strong></td>
...
if (! empty($physical_disks)) {
    ...
    foreach ($physical_disks as $pd) {
        $pd_id    = 'c' . $pd['controller'] . '_' . preg_replace('/[^a-zA-Z0-9_]/', '_', $pd['eid_slot']);
        $pd_label = 'c' . $pd['controller'] . ' / ' . htmlspecialchars($pd['eid_slot'])
                  . ' &mdash; ' . htmlspecialchars($pd['model'])
                  . ' (' . htmlspecialchars($pd['size']) . ')';
