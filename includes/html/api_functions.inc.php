...
function update_device_attrib(Illuminate\Http\Request $request)
{
    ...
    } elseif (dbUpdate(['attrib_value' => $data['attrib_value']], 'devices_attribs', '`device_id`=? AND `attrib_type`=?', [$device_id, $data['attrib_type']]) >= 0) {
        return api_success_noresult(200, 'Device Attrib ' . $data['attrib_type'] . ' field has been updated');
    } else {
        return api_error(500, 'Device Attrib ' . $data['attrib_type'] . ' field failed to be updated');
    }
}
