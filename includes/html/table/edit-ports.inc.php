...
foreach (dbFetchRows($sql, $param) as $port) {
    ...
    $associated_port_groups = dbFetchColumn('SELECT `port_group_id` FROM `port_group_port` where `port_id`=?', $port['port_id']) ?: ['Default'];
    ...
    $port_group_field = '<select class="input-sm" name="port_group_' . $port['port_id'] . '[]"  data-device_id="' . $port['device_id'] . '" multiple>';
