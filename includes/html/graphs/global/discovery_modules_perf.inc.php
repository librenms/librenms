...
/**
 * discovery_modules_perf.inc.php
 *
 * Global discovery per-module performance graph
 *
...
$modules = array_keys(LibrenmsConfig::get('discovery_modules'));
sort($modules);
...
foreach ($modules as $module_index => $module) {
    ...
    foreach ($hostnames as $index => $hostname) {
        $rrd_filename = Rrd::name($hostname, ['discovery-perf', $module]);
        if (Rrd::checkRrdExists($rrd_filename)) {
            $rrd_options[] = "DEF:{$module}Raw$index=$rrd_filename:discovery:AVERAGE";
            ...
            if ($graph_params->visible('previous')) {
                $rrd_options[] = "DEF:{$module}RawX$index=$rrd_filename:discovery:AVERAGE:start=$prev_from:end=$from";
                // change undefined to 0
