...
foreach ($rrd_list as $rrd) {
    if (! \App\Facades\Config::get("graph_colours.$colours_in.$iter") || ! \App\Facades\Config::get("graph_colours.$colours_out.$iter")) {
        $iter = 0;
    }

    $colour_in = \App\Facades\Config::get("graph_colours.$colours_in.$iter");
    $colour_out = \App\Facades\Config::get("graph_colours.$colours_out.$iter");
...
if (! $nototal) {
    ...
    $rrd_options .= ' VDEF:percentile_in=inbits,' . \App\Facades\Config::get('percentile_value') . ',PERCENT';
    $rrd_options .= ' VDEF:percentile_out=outbits,' . \App\Facades\Config::get('percentile_value') . ',PERCENT';
    $rrd_options .= ' CDEF:dpercentile_outn=doutbits,' . $stacked['stacked'] . ',* VDEF:dpercentile_outnp=dpercentile_outn,' . \App\Facades\Config::get('percentile_value') . ',PERCENT CDEF:dpercentile_outnpn=doutbits,doutbits,-,dpercentile_outnp,' . $stacked['stacked'] . ',*,+ VDEF:dpercentile_out=dpercentile_outnpn,FIRST';
