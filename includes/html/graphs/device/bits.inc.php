...
foreach ($ports as $port) {
    $ignore = 0;
    if (is_array(\App\Facades\Config::get('device_traffic_iftype'))) {
        foreach (\App\Facades\Config::get('device_traffic_iftype') as $iftype) {
    ...
    if (is_array(\App\Facades\Config::get('device_traffic_descr'))) {
        foreach (\App\Facades\Config::get('device_traffic_descr') as $ifdescr) {
