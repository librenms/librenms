...
foreach ($prefixes as $prefix => $numOidPrefix) {
    ...
    foreach ($walk as $index => $oid) {
        if ($oid[$prefix . 'Configured'] != '0' && ($oid[$prefix . 'SensorType'] == '2' || $oid[$prefix . 'SensorType'] == 'humidity') && $oid[$prefix . 'LiveRaw']) {
            ...
            discover_sensor($valid['sensor'], 'humidity', $device, $num_oid, $prefix . 'LiveRaw' . $index, 'webmon', $descr, '1', '1', $lowLimit, $lowWarnLimit, $highWarnlimit, $highLimit, $value, 'snmp', null, null, null, null);
        }
    }
}
