...
if ($oids) {
    ...
    foreach ($oids as $sensor_index => $data) {
        ...
        if (is_numeric($current) && $current >= 0) {
            discover_sensor($valid['sensor'], 'humidity', $device, $humidity_oid, $sensor_index, 'sentry3', $descr, $divisor, $multiplier, $low_limit, $low_warn_limit, $high_warn_limit, $high_limit, $current);
        }
    }
}
