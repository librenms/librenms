...
/*
 * AMD GPU core clock, via the 'amdgpu' SNMP extend script.
 *
 ...
 * reach LM-SENSORS-MIB. The extend script reads it from sysfs.
 *
 ...
 *     30215000                       power1_average, microwatts  (power class)
 *     40067000                       power1_input, microwatts    (power class)
 *     2700000000                     freq1_input, hertz
 *
...
if (! empty($amdgpu_data)) {
    ...
    for ($amdgpu_base = 1; isset($amdgpu_lines[$amdgpu_base]); $amdgpu_base += 4) {
        ...
        foreach ($amdgpu_frequency as $amdgpu_offset => $amdgpu_descr) {
