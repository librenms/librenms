...
if ($device['os'] == 'comware') {
    ...
    $entphydata = dbFetchRows(
        "SELECT `entPhysicalIndex`, `entPhysicalClass`, `entPhysicalName` FROM `entPhysical` WHERE `device_id` = ? AND `entPhysicalClass` REGEXP 'module|sensor' ORDER BY `entPhysicalIndex`",
    ...
    foreach ($comware_oids as $index => $entry) {
        if (is_numeric($entry['hh3cTransceiverTemperature']) && $entry['hh3cTransceiverTemperature'] != 2147483647) {
            ...
            $dbquery                   = dbFetchRows(
                "SELECT `ifDescr` FROM `ports` WHERE `ifIndex`= ? AND `device_id` = ? AND `ifAdminStatus` = 'up'",
                array( $index,
                $device['device_id']
                )
            );
