...
if (isset($oids) && $oids) {
    ...
    foreach (explode("\n", $oids) as $data) {
        ...
        if ($data) {
            ...
            discover_sensor(null, \LibreNMS\Enum\Sensor::CURRENT, $device, $current_oid, $index, $type, $descr, '10', '1', $lowlimit, null, $warnlimit, $limit, $current);
        }
    }
}
unset($oids);
...
if (isset($in_oids)) {
    foreach ($in_oids as $index => $data) {
        ...
        if (substr((string) $index, 0, 1) == 2 && $data['upsPhaseInputCurrent'] != -1) {
            ...
            discover_sensor(null, \LibreNMS\Enum\Sensor::CURRENT, $device, $current_oid, $in_index, $type, $descr, $divisor, 0, null, null, null, null, $current);
...
foreach ($oids as $index => $data) {
    ...
    if ($current >= -1) {
        discover_sensor(null, \LibreNMS\Enum\Sensor::CURRENT, $device, $current_oid, $index, $type, $descr, $divisor, 1, null, null, null, null, $current);
    }
}
unset($index);
unset($data);
