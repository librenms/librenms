...
foreach ($pre_cache['sdbMgmtCtrlDevUnitAddress'] as $sdbMgmtCtrlDevUnitAddress => $sdbDevIdIndex) {
    foreach ($pre_cache['sdbDevInKWhTotal'][$sdbDevIdIndex] as $sdbDevInIndex => $sdbDevInKWhTotal) {
        ...
        $descr            = ($name != '' ? $name : "$serial_input Lifetime kWh Total");
    ...
    foreach ($pre_cache['sdbDevOutMtKWhTotal'][$sdbDevIdIndex] as $sdbDevOutMtIndex => $sdbDevOutMtKWhTotal) {
        $name             = trim($pre_cache['sdbDevOutName'][$sdbDevIdIndex][$sdbDevOutMtIndex], '"');
        $power_consumed_oid = ".1.3.6.1.4.1.31034.12.1.1.2.7.2.1.2.$sdbDevIdIndex.$sdbDevOutMtIndex";
        $serial_input     = $pre_cache['sdbDevIdSerialNumber'][$sdbDevIdIndex] ." Outlet ". $sdbDevOutMtIndex;
        $descr            = ($name != '' ? $name : "$serial_input Lifetime kWh Total");

        // See includes/discovery/entity-physical/schleifenbauer.inc.php for an explanation why we set this as the entPhysicalIndex.
        $entPhysicalIndex = $sdbMgmtCtrlDevUnitAddress * 1000000 + 100000 + $sdbDevOutMtIndex * 1000 + 140;
