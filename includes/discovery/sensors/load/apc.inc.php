...
if ($phasecount > 1) {
    ...
    foreach ($oids as $index => $data) {
        ...
        if ($load >= 0) {
            discover_sensor(null, \LibreNMS\Enum\Sensor::LOAD, $device, $load_oid, $index, $type, $descr, $divisor, 1, null, null, null, null, $load);
        }
    }
    unset($oids);
} else {
    ...
    foreach ($oid_array as $item) {
        ...
        if (! empty($oids)) {
            ...
            discover_sensor(null, \LibreNMS\Enum\Sensor::LOAD, $device, $current_oid . '.' . $item['index'], $current_oid . '.' . $item['index'], $item['type'], $item['descr'], $item['divisor'], 1, null, null, null, null, $current);
        }
    }//end foreach
}
