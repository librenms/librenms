...
foreach ($pre_cache['wipipe_oids'] as $index => $entry) {
    ...
    if ($entry['mdmStatus']) {
        ...
        //Discover Sensors
        discover_sensor(null, \LibreNMS\Enum\Sensor::STATE, $device, $cur_oid . $index, $index, $state_name, $descr, 1, 1, null, null, null, null, $entry['mdmStatus'], 'snmp', $index);
    }
}
...
foreach ($upgradestatus as $index => $entry) {
    ...
    //Discover Sensors
    discover_sensor(null, \LibreNMS\Enum\Sensor::STATE, $device, $cur_oid . $index, $index, $state_name, $descr, 1, 1, null, null, null, null, $entry['devFWUpgradeStatus'], 'snmp', $index);
}
