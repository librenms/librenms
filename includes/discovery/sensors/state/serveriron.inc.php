...
for ($i = 1; $i != 3; $i++) {
    ...
    if (! empty($power_status)) {
        discover_sensor(null, \LibreNMS\Enum\Sensor::STATE, $device, $power_oid, 'powerstatus' . $i, 'snmp', 'Power Supply ' . $i . ' Status', 1, 1, '1', null, null, '3', $power_status);
    }
}
...
for ($i = 1; $i != 7; $i++) {
    ...
    if (! empty($fan_status)) {
        discover_sensor(null, \LibreNMS\Enum\Sensor::STATE, $device, $fan_oid, 'fanstatus' . $i, 'snmp', 'Fan ' . $i . ' Status', 1, 1, '1', null, null, '3', $fan_status);
    }
}
