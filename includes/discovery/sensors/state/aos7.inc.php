...
foreach ($pre_cache['aos7_fan_oids'] as $index => $data) {
   if (is_array($data)) {
     ...
     echo "value $value \n";
     echo "revindex $revindex  \n";
     echo "revchass $revchass \n";
     echo "revdata $revdata \n";
...
echo "Checking Chassis states \n";
        ...
        preg_match('/chasEntPhysOperStatus.(2.+) = (.+)/', $chas_entry, $data2); // entPhysicalName.284 = "5/PS-2"
         ...
         $number = $data2[1];
	 $value = $data2[2];
	 echo ("chasEntPhysOperStatus.$number = $value\n");
