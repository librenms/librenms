...
if (isset($oids) && is_array($oids)) {
    ...
    foreach ($oids as $index => $entry) {
        //Discover Sensors
        discover_sensor(null, \LibreNMS\Enum\Sensor::STATE, $device, $cur_oid . $index, $index, $state_name, 'Temperature Sensor ' . $index, 1, 1, null, null, null, null, $entry['tempSensorStatus'], 'snmp', $index);
    }
}
...
if (is_array($oids)) {
    ...
    foreach ($oids as $index => $entry) {
        //Discover Sensors
        discover_sensor(null, \LibreNMS\Enum\Sensor::STATE, $device, $cur_oid . $index, $index, $state_name, 'Storage Status: ' . ($entry['storageName'] ?? null), 1, 1, null, null, null, null, $entry['storageDisruptionDetected'], 'snmp', $index);
    }
}
