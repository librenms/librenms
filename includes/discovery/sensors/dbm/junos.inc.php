...
foreach ($pre_cache['junos_ifoptics_oids'] as $index => $entry) {
    if (is_numeric($entry['jnxPMCurRxInputPower'])) {
        ...
        $limit_low = $pre_cache['junos_ifoptics2_oids'][$alt_index]['jnxRxPowerLowThresh']/$divisor;
        $warn_limit_low = null;
        $limit = $pre_cache['junos_ifoptics2_oids'][$alt_index]['jnxRxPowerHighThresh']/$divisor;
        $warn_limit = null;
        $current = $entry['jnxPMCurRxInputPower']/$divisor;
        $entPhysicalIndex = $index;
        $entPhysicalIndex_measured = 'ports';
        discover_sensor($valid['sensor'], 'dbm', $device, $oid, 'rx-'.$index, 'junos', $descr, $divisor, $multiplier, $limit_low, $warn_limit_low, $warn_limit, $limit, $current, 'snmp', $entPhysicalIndex, $entPhysicalIndex_measured);
    }
    if (is_numeric($entry['jnxPMCurTxOutputPower'])) {
        ...
        $limit_low = $pre_cache['junos_ifoptics2_oids'][$alt_index]['jnxTxPowerLowThresh']/$divisor;
        $warn_limit_low = null;
        $limit = $pre_cache['junos_ifoptics2_oids'][$alt_index]['jnxRxPowerHighThresh']/$divisor;
        $warn_limit = null;
        $current = $entry['jnxPMCurTxOutputPower']/$divisor;
        $entPhysicalIndex = $index;
        $entPhysicalIndex_measured = 'ports';
        discover_sensor($valid['sensor'], 'dbm', $device, $oid, 'tx-'.$index, 'junos', $descr, $divisor, $multiplier, $limit_low, $warn_limit_low, $warn_limit, $limit, $current, 'snmp', $entPhysicalIndex, $entPhysicalIndex_measured);
    }
