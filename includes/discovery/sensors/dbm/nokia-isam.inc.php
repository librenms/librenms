...
foreach ($pre_cache['nokiaIsamSfpPort'] as $slotId => $slot) {
    ...
    foreach ($slot as $portId => $port) {
        ...
        if (isset($port['sfpDiagRxPower']) && is_numeric($port['sfpDiagRxPower'])) {
            ...
            discover_sensor($valid['sensor'], 'dbm', $device, $oid, $portName . '-rx', 'nokia-isam', $descr, $divisor, $multiplier, $limit_low, $warn_limit_low, $warn_limit, $limit, $value);
        }
        ...
        if (isset($port['sfpDiagTxPower']) && is_numeric($port['sfpDiagTxPower'])) {
            ...
            discover_sensor($valid['sensor'], 'dbm', $device, $oid, $portName . '-tx', 'nokia-isam', $descr, $divisor, $multiplier, $limit_low, $warn_limit_low, $warn_limit, $limit, $value);
        }
    }
}
