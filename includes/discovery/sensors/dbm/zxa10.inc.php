...
foreach ($pre_cache["zxa10_oids"] as $index => $entry) {
    ...
    if ($entry["zxAnOpticalIfRxPwrCurrValue"]) {
        ...
        discover_sensor(
            ...
            "dbm",
            $device,
            ".1.3.6.1.4.1.3902.1082.30.40.2.4.1.2." . $index,
            "zxAnOpticalIfRxPwrCurrValue." . $index,
            "zxa10_olt_from_onu_rx",
            $descr,
            $divisor,
            $multiplier,
            "-34",
...
foreach ($pre_cache["zxa10_onu_oids"] as $index => $entry) {
    ...
    if ($entry["zxAnPonRxOpticalPower"]) {
        $i = explode(".", $index);
        list($dec, $onu_num) = $i;
        foreach ($gpon_onu_id as $key => $value) {
            if ($dec == $value) {
                $port = $key;
            }
        }
        ...
        discover_sensor(
            ...
            "dbm",
            $device,
            ".1.3.6.1.4.1.3902.1082.500.1.2.4.2.1.2." . $index,
            "zxAnPonRxOpticalPower." . $index,
            "zxa10_onu_from_olt_rx",
            $descr,
            $divisor,
            $multiplier,
            "-34",
