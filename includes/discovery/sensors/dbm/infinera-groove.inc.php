...
foreach ($pre_cache['infineragroove_portTable'] as $index => $data) {
    ...
    if (isset($data['portRxOpticalPower']) && is_numeric($data['portRxOpticalPower']) && in_array($data['portAdminStatus'], ['up', '3'], true)) {
        ...
        discover_sensor(null, \LibreNMS\Enum\Sensor::DBM, $device, $oid, 'portRxOpticalPower.' . $index, 'infinera-groove', $descr, $divisor, '1', null, null, null, null, $value, 'snmp', null, null, null, $portAlias, 'GAUGE');
    }
    ...
    if (isset($data['portTxOpticalPower']) && is_numeric($data['portTxOpticalPower']) && in_array($data['portAdminStatus'], ['up', '3'], true)) {
        ...
        discover_sensor(null, \LibreNMS\Enum\Sensor::DBM, $device, $oid, 'portTxOpticalPower.' . $index, 'infinera-groove', $descr, $divisor, '1', null, null, null, null, $value, 'snmp', null, null, null, $portAlias, 'GAUGE');
    }
}
