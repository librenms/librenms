...
foreach (explode("\n", $oids) as $data) {
    ...
    if ($data) {
        ...
        if ($temperature != -128) { // -128 = not measured/present
            discover_sensor(null, \LibreNMS\Enum\Sensor::TEMPERATURE, $device, $temperature_oid, Str::padLeft($temperature_id, 2, '0'), 'areca', $descr, '1', '1', null, null, null, null, $temperature);
        }
    }
}
...
foreach (explode("\n", (string) $oids) as $data) {
    ...
    if ($data) {
        ...
        discover_sensor(null, \LibreNMS\Enum\Sensor::TEMPERATURE, $device, $oid, $index, 'areca', trim($descr, '"'), '1', '1', null, null, null, null, $current);
    }
}
