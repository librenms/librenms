...
if (is_numeric($value)) {
    ...
    $descr = 'CPU Temp';
    discover_sensor(null, \LibreNMS\Enum\Sensor::TEMPERATURE, $device, $sensor_oid, 1, $sensor_type, $descr, 1, 1, null, null, null, null, $value);
}
    ...
    foreach (explode("\n", $oids) as $data) {
        ...
        if ($data != '') {
            ...
            if (! empty($temperature)) {
                discover_sensor(null, \LibreNMS\Enum\Sensor::TEMPERATURE, $device, $temperature_oid, $oid, 'hpilo', $descr, '1', '1', null, null, null, $threshold, $temperature);
            }
        }
    }
}
...
if (preg_match('/(Linux).+(ntc)/', (string) $device['sysDescr'])) {
    ...
    if (is_numeric($value)) {
        discover_sensor(null, \LibreNMS\Enum\Sensor::TEMPERATURE, $device, $oid . $index, $index, $sensor_type, $descr, '1', '1', $lowlimit, $lowwarnlimit, $warnlimit, $limit, $value);
    }
}
