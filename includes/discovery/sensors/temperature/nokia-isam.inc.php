...
foreach ($snmp_data['nokiaIsamSlotTemperature'] as $slotId => $slot) {
    ...
    foreach ($slot as $sensorId => $sensor) {
        if (isset($sensor['eqptBoardThermalSensorActualTemperature']) && is_numeric($sensor['eqptBoardThermalSensorActualTemperature'])) {
            ...
            discover_sensor($valid['sensor'], 'temperature', $device, $oid, $slotName . '.' . $sensorId . '-temp', 'nokia-isam', $descr, $divisor, $multiplier, $limit_low, $warn_limit_low, $warn_limit, $limit, $value);
        }
    }
}
