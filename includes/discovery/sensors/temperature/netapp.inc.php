...
if ($oids) {
    ...
    foreach (explode("\n", $oids) as $data) {
        ...
        preg_match_all('/([0-9]+C)+/', SnmpQuery::get($main_oid . '.26.' . $temperature_id)->value(), $over_fail);
        preg_match_all('/([0-9]+C)+/', SnmpQuery::get($main_oid . '.27.' . $temperature_id)->value(), $over_warn);
        preg_match_all('/([0-9]+C)+/', SnmpQuery::get($main_oid . '.28.' . $temperature_id)->value(), $under_fail);
        preg_match_all('/([0-9]+C)+/', SnmpQuery::get($main_oid . '.29.' . $temperature_id)->value(), $under_warn);
        $x = 0;
