...
if (! empty($pre_cache['raspberry_pi_sensors'])) {
    ...
    for ($volt = 2; $volt < 6; $volt++) {
        ...
        if (is_numeric($value)) {
            discover_sensor(null, \LibreNMS\Enum\Sensor::VOLTAGE, $device, $oid . $volt, $volt, $sensor_type, $descr, '1', '1', null, null, null, null, $value);
        } else {
            break;
        }
    }
...
foreach (explode("\n", $oids) as $data) {
    ...
    if ($data) {
        ...
        if ($kind == 1) {
            ...
            if ($monitor == 'true') {
                discover_sensor(null, \LibreNMS\Enum\Sensor::VOLTAGE, $device, $volt_oid, $index, $type, $descr, $divisor, '1', $lowlimit, null, null, $limit, $current);
            }
        }
    }//end if
}
...
if (preg_match('/(Linux).+(ntc)/', (string) $device['sysDescr'])) {
    ...
    if (is_numeric($value)) {
        discover_sensor(null, \LibreNMS\Enum\Sensor::VOLTAGE, $device, $oid . $index, $index, $sensor_type, $descr, '1', '1', $lowlimit, $lowwarnlimit, $warnlimit, $limit, $value);
    }
    $descr = 'VBUS voltage';
    $index = '116.4';
    $value = snmp_get($device, $oid . $index, '-Oqv');
