...
foreach ($pre_cache['aos7_oids'] as $index => $entry) {
    if (is_numeric($entry['ddmPortSupplyVoltage']) && $entry['ddmPortSupplyVoltage'] != 0) {
        ...
        $limit_low = ($port['ddmPortSupplyVoltageLowAlarm']/$divisor);
        $warn_limit_low = ($port['ddmPortSupplyVoltageLowWarning']/$divisor);
        $limit = ($port['ddmPortSupplyVoltageHiAlarm']/$divisor);
        $warn_limit = ($port['ddmPortSupplyVoltageHiWarning']/$divisor);
        $value = $port['ddmPortSupplyVoltage']/$divisor;
