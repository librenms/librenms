...
if ($device['os'] == 'roomalert3e') {
	...
	$description		=	snmp_get($device, $description_oid, '-OsqnU', 'ROOMALERT12E-MIB', '+'.$config['install_dir'].'/mibs/avtech');
	$value_oid			=	".1.3.6.1.4.1.20916.1.9.1.1.1.1.0";
	$value				=	snmp_get($device, $value_oid, '-OsqnU', 'ROOMALERT12E-MIB', '+'.$config['install_dir'].'/mibs/avtech');
	$divisor			=	100;
	$multiplier			=	1;
	if (is_numeric($value)) {
