...
if ($device['os'] == 'areca') {
    ...
    foreach (explode("\n", $oids) as $data) {
        ...
        if ($data) {
            ...
            if ($temperature != -128) { // -128 = not measured/present
                discover_sensor($valid['sensor'], 'temperature', $device, $temperature_oid,
                    zeropad($temperature_id), 'areca',
    ...
    foreach (explode("\n", $oids) as $data) {
        ...
        if ($data) {
            ...
            discover_sensor($valid['sensor'], 'temperature', $device, $oid,
                $index, 'areca',
                trim($descr, '"'), '1', '1', null, null, null, null, $current);
        }
    }
}
