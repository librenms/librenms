...
if (mysql_result(mysql_query("SELECT COUNT(*) FROM `sensors` WHERE `device_id` = '".$device['device_id']."' AND `sensor_class` = 'temperature' AND (`sensor_type` = 'cisco-entity-sensor' OR `sensor_type` = 'entity-sensor')"),0) == "0" && (in_array($device['os'], array("ios", "catos")) || in_array($device['os_group'], array("ios", "catos"))))
{
  echo("Cisco ");
