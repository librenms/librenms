...
for($i = 1; $i <= $num_cpu; $i++) {
        ...
        $proc_type_oid = snmp_get($device, "FORTINET-FORTIGATE-MIB::fgProcessorType.$i", '-Ovq');
        // TODO: Pretty up the processor descr
        $descr = explode ( "::", $proc_type_oid)[1];
        print "CPU: $num_cpu - USAGE: $cpu_usage - TYPE $descr\n";
        if (is_numeric($usage)) {
                discover_processor($valid['processor'], $device, "FORTINET-FORTIGATE-MIB::fgProcessorUsage." . $num_cpu, $i, 'fortigate-fixed', $descr, '1', $usage, null, null);
        }
