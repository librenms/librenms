...
function get_device_divisor($device, $os_version, $sensor_type, $oid)
{
    if ($device['os'] == 'poweralert') {
        ...
        if ($sensor_type == 'frequency') {
            if (\Illuminate\Support\Str::startsWith($device['hardware'], "UPS2000")) {
                return 10;
            }
            return 100;
        }
    } elseif ($device['os'] == 'hpe-rtups') {
        if ($sensor_type == 'voltage' && !\Illuminate\Support\Str::startsWith($oid, '.1.3.6.1.2.1.33.1.2.5.') && !\Illuminate\Support\Str::startsWith($oid, '.1.3.6.1.2.1.33.1.3.3.1.3')) {
            return 1;
        }
    ...
    if ($sensor_type == 'voltage' && !\Illuminate\Support\Str::startsWith($oid, '.1.3.6.1.2.1.33.1.2.5.')) {
        return 1;
    }

    if ($sensor_type == 'runtime') {
        if (\Illuminate\Support\Str::startsWith($oid, '.1.3.6.1.2.1.33.1.2.2.')) {
            return 60;
        }

        if (\Illuminate\Support\Str::startsWith($oid, '.1.3.6.1.2.1.33.1.2.3.')) {
...
function ignore_storage($os, $descr)
{
    ...
    foreach (Config::getOsSetting($os, 'ignore_mount_string') as $ims) {
        if (\Illuminate\Support\Str::contains($descr, $ims)) {
            ...
            foreach ($raw_data as $index => $snmp_data) {
                ...
                if (!is_numeric($snmp_value)) {
                    if ($sensor_type === 'temperature') {
                        ...
                        if (\Illuminate\Support\Str::endsWith($snmp_value, array('f', 'F'))) {
