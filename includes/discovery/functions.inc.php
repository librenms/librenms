        ...
        foreach ($discovery[$sensor_class]['data'] as $data) {
            ...
            foreach ($raw_data as $index => $snmp_data) {
                ...
                if ($skippedFromYaml === false && is_numeric($value)) {
                    ...
                    $divisor = (int) (isset($data['divisor']) ? YamlDiscovery::fillValues('divisor', $index, $count, $data, $pre_cache, $data['divisor']) : ($sensor_options['divisor'] ?? 1));
                    $multiplier = (int) (isset($data['multiplier']) ? YamlDiscovery::fillValues('multiplier', $index, $count, $data, $pre_cache, $data['multiplier']) : ($sensor_options['multiplier'] ?? 1));
