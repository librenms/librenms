...
// Discover sensors
function discover_sensor(&$valid, $class, $device, $oid, $index, $type, $descr, $divisor = 1, $multiplier = 1, $low_limit = null, $low_warn_limit = null, $warn_limit = null, $high_limit = null, $current = null, $poller_type = 'snmp', $entPhysicalIndex = null, $entPhysicalIndex_measured = null, $user_func = null, $group = null, $offset = 0)
{
    ...
    if (dbFetchCell('SELECT COUNT(sensor_id) FROM `sensors` WHERE `poller_type`= ? AND `sensor_class` = ? AND `device_id` = ? AND sensor_type = ? AND `sensor_index` = ?', array($poller_type, $class, $device['device_id'], $type, (string)$index)) == '0') {
        ...
        if ($oid == $sensor_entry['sensor_oid'] &&
            ...
            $offset == $sensor_entry['sensor_offset'] &&
        ...
        foreach ($device['dynamic_discovery']['modules']['sensors'][$sensor_type]['data'] as $data) {
            ...
            foreach ($raw_data as $index => $snmp_data) {
                ...
                if (YamlDiscovery::canSkipItem($value, $index, $data, $sensor_options, $pre_cache) === false && is_numeric($value)) {
                    ...
                    $offset = $data['offset'] ?: ($sensor_options['offset'] ?: 0);
                    ...
                    foreach ($limits as $limit) {
                        if (is_numeric($data[$limit])) {
                            ...
                            if (is_numeric($$limit)) {
                                $$limit = ($$limit / $divisor) * $multiplier + $offset;
                            }
                        }
                    }
                    ...
                    if ($sensor_type === 'state') {
                        ...
                        $value = ($value / $divisor) * $multiplier + $offset;
                    }

                    $uindex = str_replace('{{ $index }}', $index, isset($data['index']) ? $data['index'] : $index);
                    discover_sensor($valid['sensor'], $sensor_type, $device, $oid, $uindex, $sensor_name, $descr, $divisor, $multiplier, $low_limit, $low_warn_limit, $warn_limit, $high_limit, $value, 'snmp', $entPhysicalIndex, $entPhysicalIndex_measured, $user_function, $group, $offset);
