...
foreach ($systemd_mapper as $state_type => $state_statuses) {
    ...
    foreach ($systemd_mapper[$flattened_type] as $state_status => $rrd_location) {
        $field_value = null;
        $field_name = $state_status;

        if (empty($sub_state_type)) {
            $field_value = $systemd_data[$state_type][$state_status] ?? null;
        } else {
            $field_value = $systemd_data[$state_type][$sub_state_type][$state_status] ?? null;
        }
        ...
        if (! is_int($field_value) && ! is_null($field_value)) {
            ...
            $flattened_type . ' state with' . $state_status . ' state status: ' . $field_value;
        ...
        if ($rrd_location === 'individual') {
            ...
            $metrics = systemd_data_update_helper($device, $app->app_id, $individual_fields, $metrics, $name, $polling_type, $individual_rrd_def, $rrd_flattened_type);
            continue;
        }
    ...
    $metrics = systemd_data_update_helper($device, $app->app_id, $shared_fields, $metrics, $name, $polling_type, $shared_rrd_def, $flattened_type);
}
