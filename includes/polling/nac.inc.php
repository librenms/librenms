...
if (!get_dev_attrib($device, 'poll_cisco-nac', 'false')) {
    echo "\nCisco-NAC\n";
    ...
    if (!empty($portAuthSessionEntry)) {
        $cafSessionMethodsInfoEntry = collect(snmpwalk_cache_oid($device, 'cafSessionMethodsInfoEntry', array(), 'CISCO-AUTH-FRAMEWORK-MIB'))->mapWithKeys(function ($item, $key) {
