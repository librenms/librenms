...
if ($device['os'] === 'f5' && (version_compare($device['version'], '11.2.0', '>=') && version_compare($device['version'], '11.7', '<'))) {
    ...
    if ($config['polling']['selected_ports'] === true || $device['attribs']['selected_ports'] == 'true' && dbFetchCell("SELECT (count(*) / (SELECT count(*) from `ports` WHERE `device_id` = ?))> 0.33 FROM `ports` WHERE `device_id`= ? AND `deleted` = 1 HAVING count(*) > 10", array($device['device_id'],$device['device_id']))) {
