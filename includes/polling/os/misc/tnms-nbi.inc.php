...
function SqlNEAlarmCreate($table, $alarmmib, $device)
{
    ...
    $neids = TnmsNeInfo::where('device_id', $device['device_id'])->pluck('neID');
    $existing = TnmsAlarm::where('device_id', $device['device_id'])->get()->keyBy('timestamp');
        foreach ($ne_alarm_oids as $alarm) {
        if ($existing_alarm = $existing->get($alarm['enmsAlTimeStamp'])) {
            $existing_alarm->file($alarm);
            $existing_alarm->save();
            $existing->forget($alarm['enmsAlTimeStamp']);
        }
            else {
            $tnms_alarm = new TnmsAlarm($alarm);
            $tnms_alarm->tnmsne_info_id = $neids->get($alarm['enmsAlNEId']);
            $tnms_alarm->device_id = $device['device_id'];
            $tnms_alarm->alarm_cause = $neids->get($alarm['enmsAlProbableCauseString']);
            $tnms_alarm->alarm_location = $neids->get($alarm['enmsAlAffectedLocation']);
            $tnms_alarm->neAlarmtimestamp = $neids->get($alarm['enmsAlTimeStamp']);
            $tnms_alarm->save();
        }
    }
    // delete old ones
