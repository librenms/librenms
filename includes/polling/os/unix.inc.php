...
if ($device['os'] == "linux" || $device['os'] == "endian") {
    ...
    if (!$hw) {
        ...
        $hardware_dmide = str_replace("\"", "", snmp_get($device, ".1.3.6.1.4.1.2021.7890.3.4.1.2.12.109.97.110.117.102.97.99.116.117.114.101.114.1", "-Oqv", "NET-SNMP-EXTEND-MIB"));
    ...
    if (!$serial) {
        $serial = str_replace("\"", "", snmp_get($device, ".1.3.6.1.4.1.2021.7890.4.4.1.2.6.115.101.114.105.97.108.1", "-Oqv", "NET-SNMP-EXTEND-MIB"));
        $serial = trim(str_replace("\"", "", $serial));
    }
