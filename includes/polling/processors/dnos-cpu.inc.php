...
if ($device['os'] == 'dnos') {
    ...
    if ($series == 'f10SSeriesProducts') {
        $proc = trim(snmp_get($device, 'F10-S-SERIES-CHASSIS-MIB::chStackUnitCpuUtil5Sec.1', '-OvQ'));
    } elseif ($series == 'f10CSeriesProducts') {
        $proc = trim(snmp_get($device, 'F10-C-SERIES-CHASSIS-MIB::chLineCardCpuUtil5Sec.1', '-OvQ'));
    } else {
