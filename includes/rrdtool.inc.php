...
function rrdtool_running($process) {
    return isset($rrd_async_process) && $rrd_async_process instanceof Proc && $rrd_async_process->isRunning();
}
...
function rrdtool_graph($graph_file, $options)
{
    ...
    if (rrdtool_initialize($config['rrdcached'])) {
        ...
        if ($config['rrdcached']) {
            ...
            $rrd_async_process->sendInput('graph --daemon ' . $config['rrdcached'] . " $graph_file $options");
        } else {
            $rrd_async_process->sendInput("graph $graph_file $options");
        }

//        $line = "";
//        $data = "";
//        while (strlen($line) < 1) {
//            $line = fgets($rrd_async_process->stdout(), 1024);
//            $data .= $line;
//        }
        ...
        if ($debug) {
            ...
            echo '</p><p>';
            echo "command returned $return_value ($output)\n";
            echo '</p>';
        }
