...
function snmp_get_multi($device, $oids, $options = '-OQUs', $mib = null, $mibdir = null, $array = array())
{
    ...
    foreach (explode("\n", $data) as $entry) {
        ...
        if (!\Illuminate\Support\Str::contains($value, 'at this OID')) {
            if (is_null($index)) {
                if (empty($oid)) {
...
function snmp_get_multi_oid($device, $oids, $options = '-OUQn', $mib = null, $mibdir = null)
{
    ...
    foreach ($data as $entry) {
        if (\Illuminate\Support\Str::contains($entry, '=')) {
            list($oid,$value)  = explode('=', $entry, 2);
            $oid               = trim($oid);
            $value             = trim($value, "\\\" \n\r");
...
function snmp_walk($device, $oid, $options = null, $mib = null, $mibdir = null)
{
    ...
    if (is_string($data) && (preg_match('/No Such (Object|Instance)/i', $data) || preg_match('/Wrong Type(.*)should be/', $data))) {
        ...
        if (\Illuminate\Support\Str::endsWith($data, '(It is past the end of the MIB tree)')) {
...
function snmpwalk_group($device, $oid, $mib = '', $depth = 1, $array = array(), $mibdir = null, $strIndexing = null)
{
    ...
    $line = strtok($data, "\n");
    while ($line !== false) {
        if (\Illuminate\Support\Str::contains($line, 'at this OID') || \Illuminate\Support\Str::contains($line, 'this MIB View')) {
            $line = strtok("\n");
            continue;
        }
        ...
        while ($line !== false && !\Illuminate\Support\Str::contains($line, '=')) {
            $value .= $line . PHP_EOL;
            $line = strtok("\n");
        }
