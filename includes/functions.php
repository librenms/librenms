...
function parse_modules($type, $options)
{
    ...
    if ($options['m']) {
        ...
        foreach (explode(',', $options['m']) as $module) {
            ...
            if (\Illuminate\Support\Str::contains($module, '/')) {
...
function checkDiscovery($device, $array)
{
    ...
    foreach ($array as $key => $value) {
        if ($check = \Illuminate\Support\Str::endsWith($key, '_except')) {
            $key = substr($key, 0, -7);
        }

        if ($key == 'sysObjectID') {
            if (\Illuminate\Support\Str::startsWith($device['sysObjectID'], $value) == $check) {
                return false;
            }
        } elseif ($key == 'sysDescr') {
            if (\Illuminate\Support\Str::contains($device['sysDescr'], $value) == $check) {
                return false;
            }
...
function compare_var($a, $b, $comparison = '=')
{
    switch ($comparison) {
        ...
        case "<":
            return $a < $b;
        case "contains":
            return \Illuminate\Support\Str::contains($a, $b);
        case "not_contains":
            return !\Illuminate\Support\Str::contains($a, $b);
        case "starts":
            return \Illuminate\Support\Str::startsWith($a, $b);
        case "not_starts":
            return !\Illuminate\Support\Str::startsWith($a, $b);
        case "ends":
            return \Illuminate\Support\Str::endsWith($a, $b);
        case "not_ends":
            return !\Illuminate\Support\Str::endsWith($a, $b);
        case "regex":
...
function getLogo($device)
{
    ...
    if (!\Illuminate\Support\Str::startsWith($img, 'generic')) {
...
function is_port_valid($port, $device)
{
    ...
    foreach (Config::getCombined($device['os'], 'bad_iftype') as $bt) {
        if (\Illuminate\Support\Str::contains($ifType, $bt)) {
    ...
    foreach (Config::getCombined($device['os'], 'bad_ifoperstatus') as $bos) {
        if (\Illuminate\Support\Str::contains($ifOperStatus, $bos)) {
...
function get_toner_levels($device, $raw_value, $capacity)
{
    ...
    if ($device['os'] == 'ricoh' || $device['os'] == 'nrg' || $device['os'] == 'lanier') {
        ...
        if (!\Illuminate\Support\Str::contains($device['hardware'], 'MFC-L8850')) {
            switch ($raw_value) {
                case '0':
                    return 100;
                case '1':
