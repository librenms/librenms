...
CISCO-ERR-DISABLE-MIB::cErrDisableIfStatusCause.$port->ifIndex.0 bpduGuard";
    ...
    {
        $device = Device::factory()->create(); /** @var Device $device */
        $port = Port::factory()->make(['ifIndex' => 1]); /** @var Port $port */
        $device->ports()->save($port);
        $trapText = "$device->hostname
UDP: [$device->ip]:57602->[10.0.0.1]:162
SNMPv2-MIB::sysUpTime.0 18:30:30.32
...
CISCO-ERR-DISABLE-MIB::cErrDisableIfStatusCause.10.0 bpduGuard";
        $trap = new Trap($trapText);
        $message = 'SNMP TRAP: bpduGuard error detected on unknown port. Either ifIndex is not found in the trap, or it does not match a port on this device.';
        \Log::shouldReceive('event')->once()->with($message, $device->device_id, 'trap', 4);
        $this->assertTrue(Dispatcher::handle($trap), 'Could not handle testErrDisableBadIfIndex trap');
    }
}
