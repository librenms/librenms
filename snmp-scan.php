...
function perform_snmp_scan($net, $force_network, $force_broadcast, $transport_protocol)
{
    ...
    while ($start++ < $end) {
        ...
        foreach ($transport_protocol as $transport) {
            try {
...
$quiet = 1;
$net   = false;
$transport_protocol = array('udp','tcp');
...
if (isset($opts['h']) || (empty($opts) && (!isset($config['nets']) || empty($config['nets'])))) {
    ...
    echo '  -t                Select transport protocol. Ex : -t udp or -t tcp, not set => both'.PHP_EOL;
    exit(0);
}
...
if (isset($opts['t'])) {
    switch ($opts['t']) {
        case 'udp':
            $transport_protocol = array('udp');
            break;
        case 'tcp':
            $transport_protocol = array('tcp');
            break;
    }
}
...
if (isset($opts['r'])) {
    ...
    if (ip2long($net->network) !== false) {
        perform_snmp_scan($net, $force_network, $force_broadcast, $transport_protocol);
