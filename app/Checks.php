    ...
    public static function postAuth()
    {
        ...
        if ($user->isAdmin()) {
            ...
            if (Device::isUp()->where('last_polled', '<=', Carbon::now()->subSeconds($warn_sec))->exists()) {
                ...
                Toastr::warning('<a href="poller-groups?tab=log&filter=unpolled/">It appears as though you have some devices that haven\'t completed polling within the last ' . $warn_min . ' minutes, you may want to check that out :)</a>', 'Devices unpolled');
            }
