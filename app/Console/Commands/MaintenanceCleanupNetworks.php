...
class MaintenanceCleanupNetworks extends LnmsCommand
{
    ...
    public function handle(): int
    {
        ...
        if (LibrenmsConfig::get('networks_purge') === true || $force) {
            ...
            $oldIpv6Networks = Ipv6Network::withCount('ipv6')->having('ipv6_count', 0)->pluck('ipv6_network_id');
            if ($oldIpv6Networks->count() > 0) {
                $this->line(trans('commands.maintenance:cleanup-networks.delete', ['count' => $oldIpv6Networks->count()]));
