...
class SensorObserver
{
    ...
    public function updating(Sensor $sensor)
    {
        ...
        if ($sensor->sensor_custom === 'Yes') {
            ...
            $clearLegacyTemperatureLowLimit = self::hasLegacyGuessedTemperatureLowLimit($sensor);

            // change unset sensor limits to current values
            $sensor->sensor_limit ??= $sensor->getOriginal('sensor_limit');
            $sensor->sensor_limit_warn ??= $sensor->getOriginal('sensor_limit_warn');
            $sensor->sensor_limit_low_warn ??= $sensor->getOriginal('sensor_limit_low_warn');
            if (! $clearLegacyTemperatureLowLimit) {
                $sensor->sensor_limit_low ??= $sensor->getOriginal('sensor_limit_low');
            }
        }
    }
    ...
    /**
     * Detect low temperature limits generated by the previous generic guess.
     *
     * The old low default was current - 10. When the high limit was guessed
     * too, the pair also has a distinctive 30 degree gap. Check both shapes
     * because some sensors supply an explicit high limit. Only remove the old
     * value when discovery no longer supplies a low limit. Explicit hardware
     * limits and user-customized limits are therefore preserved.
     */
    private static function hasLegacyGuessedTemperatureLowLimit(Sensor $sensor): bool
    {
        $high = $sensor->getOriginal('sensor_limit');
        $low = $sensor->getOriginal('sensor_limit_low');
        $current = $sensor->getOriginal('sensor_current');

        $matchesOldLowGuess = $current !== null
            && abs(((float) $current - (float) $low) - 10.0) < 0.000001;
        $matchesOldLimitPair = $high !== null
            && abs(((float) $high - (float) $low) - 30.0) < 0.000001;

        return $sensor->sensor_class === 'temperature'
            && $sensor->sensor_limit_low === null
            && $sensor->sensor_limit_warn === null
            && $sensor->sensor_limit_low_warn === null
            && $low !== null
            && ($matchesOldLowGuess || $matchesOldLimitPair);
    }
}
