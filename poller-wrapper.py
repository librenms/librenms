...
scheduler = config.get("schedule_type").get("poller", "unrestricted")
enabled = True if scheduler == "unrestricted" else scheduler == "cron"
if not enabled:
    logger.debug("Polling is not enabled for cron scheduling")
    sys.exit(0)
