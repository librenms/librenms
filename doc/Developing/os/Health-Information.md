...
The only sensor we have defined here is airflow. The available options are as follows:
  ...
  - `offset` (optional): This is a signed offset added to the return value AFTER both divisor and multiplier have been processed, if given. finalValue = (snmpValue * multiplier / divisor) + offset;
...
For `options:` you have the following available:
  ...
  - `offset`: This is the signed offset to add to the returned `value` (after both multiplier and divisor have been processed). finalValue = (snmpValue * multiplier / divisor) + offset;
...
`discover_sensor()` Accepts the following arguments:
  ...
  - $offset = Defaults to 0. This signed offset is added to the returned value after both $divisor and $multiplier have been processed. finalValue = (snmpValue * multiplier / divisor) + offset;
