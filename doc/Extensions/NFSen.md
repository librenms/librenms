...
For more defaulted information on that, please see nfdump(1). Also check a path to the nfdump. Script is using by default path `/usr/bin/nfdump`, but in some instalation nfdump can be located in `usr/bin/local/bin/nfdump`. If is this your case, than just make a symlink to nfdump, for example with: `ln -s /usr/local/bin/nfdump /usr/bin/`
