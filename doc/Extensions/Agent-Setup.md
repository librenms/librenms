...
To gather data from remote systems you can use LibreNMS in combination with check_mk (included in the scripts/librenms-agent directory).
...
```shell
cp scripts/librenms-agent/check_mk_agent /usr/bin/check_mk_agent
...
```shell
cp scripts/librenms-agent/check_mk_xinetd /etc/xinetd.d/check_mk
```
...
* Copy each of the scripts from `scripts/librenms-agent/agent-local/` into `/usr/lib/check_mk_agent/local`
* And restart xinetd.
