...
There are three ways to configure this feature. First one is from general settings of a device. The other two can be done in the 'Host Dependencies' item under 'Devices' menu. In this page, you can see all devices and there parents. Clicking on the 'bin' icon will clear the dependency setting. Clicking on the 'pen' icon will let you edit or change the current setting for chosen device. There's also a 'Manage Host Depencies' button on the top. This let you set a parent for multiple devices at one. One important remark is that, this pop-up will not show you the current settings, so you might override a previous setting. 
